...
IsoWeek_T_contra = TypeVar("IsoWeek_T_contra", bound=str | date | datetime | IsoWeekProtocol, contravariant=True)
...
class ParserMixin(IsoWeekProtocol):
    ...
    def from_compact(cls: type[Self], _str: str) -> Self:
        ...
        value = "-".join(filter(None, (_str[i:j] for i, j in zip(split_idx[:-1], split_idx[1:], strict=False))))
        return cls(value)
